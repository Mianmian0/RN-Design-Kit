...
const SectionTitle = ({ children, description }: { children: React.ReactNode, description?: string }) => (
  <div className="mb-8 border-b border-border-subtle pb-4">
    <h2 className="text-2xl font-bold text-text-100 tracking-wide">{children}</h2>
    {description && <p className="mt-2 text-text-50 font-medium">{description}</p>}
  </div>
);

const ColorSwatch = ({ name, hex, className, textColor = "text-text-100" }: { name: string, hex: string, className: string, textColor?: string }) => (
  <div className="flex flex-col gap-2">
    <div className={`h-24 w-full rounded-xl shadow-lg flex items-end p-3 ${className}`}>
      <span className={`font-mono text-xs font-semibold tracking-wider ${textColor}`}>{hex}</span>
    </div>
    <div>
      <div className="text-sm font-semibold text-text-100">{name}</div>
    </div>
  </div>
);
...
const ElevationCard = ({ level, hex, className, zIndex }: { level: string, hex: string, className: string, zIndex: number }) => (
  <div 
    className={`${className} p-6 rounded-xl shadow-2xl  flex flex-col justify-between transition-transform hover:-translate-y-1`}
    style={{ zIndex, position: 'relative', marginTop: zIndex > 0 ? '-2rem' : '0', width: `${100 - (zIndex * 5)}%`, margin: '0 auto' }}
  >
    <span className="text-sm font-bold text-text-50">{level}</span>
    <span className="font-mono text-xs text-text-50 mt-4">{hex}</span>
  </div>
);
...
const TextOpacityRow = ({ label, opacity, className }: { label: string, opacity: string, className: string }) => (
  <div className="flex items-center justify-between p-4 border-b border-border-subtle last:border-0">
    ...
    <span className="font-mono text-sm text-text-50">{opacity}</span>
  </div>
);
  ...
  <div className="flex items-center gap-6 p-4 border-b border-border-subtle/50 last:border-0 hover:bg-bg-elevated-2/50 transition-colors rounded-xl">
    ...
    <div>
      <div className="flex items-baseline gap-3 mb-1">
        <h4 className="text-lg font-bold text-text-100">{size}</h4>
...
const SpacingRow = ({ name, size, px, rem }: { name: string, size: string, px: string, rem: string }) => (
  <div className="grid grid-cols-[48px_80px_48px_64px_1fr] md:grid-cols-[56px_96px_56px_72px_1fr] gap-2 md:gap-4 items-center p-3 md:p-4 hover:bg-bg-elevated-2/50 rounded-xl transition-colors border-b border-border-subtle/30 last:border-0">
    <div className="font-black text-sm md:text-base text-primary">{name}</div>
    <div className="font-mono text-sm text-text-100 font-bold">{size}</div>
    <div className="font-mono text-xs md:text-sm text-text-50">{px}</div>
    <div className="font-mono text-xs md:text-sm text-text-50">{rem}</div>
    <div className="flex items-center">
      <div className="h-4 bg-primary/20 rounded-sm border border-primary/30" style={{ width: px }}></div>
    </div>
  </div>
);
    ...
    <div className="min-h-screen bg-bg-base p-6 md:p-12 font-sans selection:bg-primary/30">
      <div className="max-w-6xl mx-auto space-y-20">
        ...
        <header className="space-y-4 pt-8">
          <div className="inline-block px-3 py-1 bg-primary/10 border border-primary/20 rounded-full text-primary text-xs font-bold tracking-widest uppercase mb-4">
            iGaming Platform UI Kit
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold text-text-100 tracking-tight uppercase">
            设计系统规范 <br/><span className="text-primary">RN DESIGN</span>
          </h1>
          <p className="text-lg text-text-50 max-w-2xl font-medium leading-relaxed">
        ...
        <section>
          ...
          <div className="bg-bg-elevated-1 p-8 rounded-2xl shadow-xl space-y-12">
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-text-50 leading-relaxed max-w-xl">
              ...
              <div className="bg-bg-base p-4 rounded-lg border border-border-subtle flex flex-col items-center justify-center min-w-[240px]">
                ...
                <div className="text-[10px] text-text-25 mt-2">X = Font Size (px) | Y = Line Height (px)</div>
              </div>
            </div>
            ...
            {/* Headings */}
            <div>
              <h3 className="text-sm font-bold text-text-50 uppercase tracking-widest mb-4 border-b border-border-subtle pb-2 flex items-center gap-2">
              ...
              <div className="space-y-4">
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-2xl font-black text-text-100">文字-标题-特大 ABCd <span className="text-primary">24px 900</span></div>
                  <span className="font-mono text-xs text-text-50">text-2xl font-black</span>
                </div>
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-lg font-semibold text-text-100">文字-标题-大 ABCd <span className="text-primary">18px 600</span></div>
                  <span className="font-mono text-xs text-text-50">text-lg font-semibold</span>
                </div>
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-base font-semibold text-text-100">文字-标题-中 ABCd <span className="text-primary">16px 600</span></div>
                  <span className="font-mono text-xs text-text-50">text-base font-semibold</span>
                </div>
                <div className="flex items-baseline justify-between pb-2">
                  <div className="text-tiny font-semibold text-text-100">文字-标题-最小 ABCd <span className="text-primary">11px 600</span></div>
                  <span className="font-mono text-xs text-text-50">text-tiny font-semibold</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-lg font-normal text-text-100">文字-大正文/登录标题 ABCd <span className="text-primary">18px 400</span></div>
                  <span className="font-mono text-xs text-text-50">text-lg font-normal</span>
                </div>
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-base font-normal text-text-100">文字-正文-一级 ABCd <span className="text-primary">16px 400</span></div>
                  <span className="font-mono text-xs text-text-50">text-base font-normal</span>
                </div>
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-sm font-normal text-text-100">文字-正文-二级 ABCd <span className="text-primary">14px 400</span></div>
                  <span className="font-mono text-xs text-text-50">text-sm font-normal</span>
                </div>
                <div className="flex items-baseline justify-between border-b border-border-subtle/30 pb-4">
                  <div className="text-xs font-normal text-text-100">文字-正文-辅助 ABCd <span className="text-primary">12px 400</span></div>
                  <span className="font-mono text-xs text-text-50">text-xs font-normal</span>
                </div>
                <div className="flex items-baseline justify-between pb-2">
                  <div className="text-tiny font-normal text-text-100">文字-正文-最小 ABCd <span className="text-primary">11px 400</span></div>
                  <span className="font-mono text-xs text-text-50">text-tiny font-normal</span>
                </div>
              </div>
            </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-6 mb-6">
            <div className="bg-bg-elevated-1 p-6 rounded-2xl ">
              <h3 className="text-2xl font-bold text-primary mb-2">Brand Text</h3>
              <div className="font-mono text-sm text-text-50 mb-4">#FF6C44</div>
              <p className="text-sm text-text-50 font-medium">用于赔率、余额、链接等需要强调的业务数据。</p>
            </div>
          </div>

          <div className="bg-bg-elevated-1 rounded-2xl  overflow-hidden">
            <TextOpacityRow label="一 文字 & 图标 (Primary Text)" opacity="#FFFFFF 100%" className="text-text-100" />
            <TextOpacityRow label="二 文字 & 图标 (Secondary Text)" opacity="#FFFFFF 50%" className="text-text-50" />
            <TextOpacityRow label="三 文字 & 图标 (Disabled/Decorative)" opacity="#FFFFFF 25%" className="text-text-25" />
          </div>
        </section>

        {/* 3. Container Elevation */}
        <section>
          <SectionTitle description="深色模式下的空间层级（Elevation）。采用右侧“纯色版”方案，通过明度的递增来体现层级的抬升，比单纯增加透明度更清晰、性能更好。">
        ...
        <section>
          ...
          <div className="space-y-8">
            <div>
              <h3 className="text-sm font-bold text-text-50 uppercase tracking-widest mb-4">品牌色 (Brand)</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="text-sm font-semibold text-text-100">主按钮渐变 (CTA)</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-sm font-bold text-text-50 uppercase tracking-widest mb-4">成功 (Success)</h3>
              ...
              <div>
                <h3 className="text-sm font-bold text-text-50 uppercase tracking-widest mb-4">错误 (Error)</h3>
                <div className="grid grid-cols-3 gap-4">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Radius Scale */}
            <div className="bg-bg-elevated-1 rounded-2xl  p-2">
            ...
            <div className="space-y-6">
              <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl">
                <h3 className="text-lg font-bold text-text-100 mb-4 flex items-center gap-2">
                  <Square size={18} className="text-primary" /> 同心圆法则演示
                </h3>
                <p className="text-sm text-text-50 mb-6 leading-relaxed">
                  当一个圆角容器嵌套在另一个圆角容器内时，为了视觉上的和谐，必须满足：<br/>
                  <code className="text-primary font-mono bg-primary/10 px-2 py-1 rounded mt-2 inline-block">内圆角 = 外圆角 - 间距 (Padding)</code>
                </p>

                {/* Demo 1: 24px outer, 16px padding, 8px inner */}
                <div className="mb-6">
                  <div className="text-xs text-text-50 mb-2 font-mono">外层 24px (3xl) - 间距 16px (p-4) = 内层 8px (lg)</div>
                  <div className="bg-bg-elevated-2 p-4 rounded-3xl ">
                    <div className="bg-primary/20 p-6 rounded-lg border border-primary/30 flex items-center justify-center">
                      <span className="text-primary font-bold text-sm">完美嵌套 (Perfect Nesting)</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <div className="text-xs text-text-50 mb-2 font-mono">外层 16px (2xl) - 间距 8px (p-2) = 内层 8px (lg)</div>
                  <div className="bg-bg-elevated-2 p-2 rounded-2xl  w-2/3">
                    <div className="bg-success/20 p-4 rounded-lg border border-success/30 flex items-center justify-center">
                      <span className="text-success font-bold text-sm">紧凑嵌套 (Compact)</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[7fr_5fr] gap-8">
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl">
              <div className="grid grid-cols-[48px_80px_48px_64px_1fr] md:grid-cols-[56px_96px_56px_72px_1fr] gap-2 md:gap-4 items-center p-3 md:p-4 border-b border-border-subtle mb-2">
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">Name</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">Class</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">Pixels</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">Rem</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">Visual</div>
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl">
                <h3 className="text-sm font-bold text-text-100 uppercase tracking-wide mb-4">应用示例：卡片内部间距 (Card Padding)</h3>
                <div className="bg-bg-elevated-2 p-6 rounded-xl  relative">
                  <div className="absolute top-0 left-0 w-full h-6 bg-primary/10 border-b border-primary/30 flex items-center justify-center text-[10px] text-primary font-mono">p-6 (24px)</div>
                  <div className="absolute top-0 left-0 w-6 h-full bg-primary/10 border-r border-primary/30 flex items-center justify-center text-[10px] text-primary font-mono" style={{ writingMode: 'vertical-rl' }}>p-6 (24px)</div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-2 relative">
                      ...
                      <div className="absolute left-0 top-4 w-full h-2 bg-primary/10 border-y border-primary/30 flex items-center justify-center text-[10px] text-primary font-mono">space-y-2 (8px)</div>
                      <div className="h-3 bg-bg-elevated-3 rounded w-1/2"></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl">
                <h3 className="text-sm font-bold text-text-100 uppercase tracking-wide mb-4">应用示例：列表项间距 (List Item Gap)</h3>
                <div className="bg-bg-elevated-2 p-4 rounded-xl  flex flex-col gap-3 relative">
                  <div className="h-10 bg-bg-elevated-3 rounded-lg flex items-center px-3 text-sm text-text-50">List Item 1</div>
                  <div className="absolute left-1/2 top-14 -translate-x-1/2 w-10 h-3 bg-primary/10 border-y border-primary/30 flex items-center justify-center text-[10px] text-primary font-mono z-10">gap-3 (12px)</div>
                  <div className="h-10 bg-bg-elevated-3 rounded-lg flex items-center px-3 text-sm text-text-50">List Item 2</div>
                  <div className="absolute left-1/2 top-[108px] -translate-x-1/2 w-10 h-3 bg-primary/10 border-y border-primary/30 flex items-center justify-center text-[10px] text-primary font-mono z-10">gap-3 (12px)</div>
                  <div className="h-10 bg-bg-elevated-3 rounded-lg flex items-center px-3 text-sm text-text-50">List Item 3</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          <div className="space-y-8">
            {/* Breakpoints Table */}
            <div className="bg-bg-elevated-1 rounded-2xl  overflow-hidden shadow-xl">
              <div className="grid grid-cols-4 gap-4 p-4 md:p-6 border-b border-border-subtle bg-bg-elevated-2/30">
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">设备 (Device)</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">断点 (Breakpoint)</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">列数 (Columns)</div>
                <div className="text-xs font-bold text-text-50 uppercase tracking-wider">边距/间距 (Margin/Gutter)</div>
              </div>
              <div className="grid grid-cols-4 gap-4 p-4 md:p-6 border-b border-border-subtle/50 hover:bg-bg-elevated-2/30 transition-colors">
                <div className="font-bold text-text-100">Mobile <span className="text-text-50 font-normal text-sm ml-2 hidden md:inline">(默认)</span></div>
                <div className="font-mono text-sm text-primary">&lt; 768px</div>
                <div className="text-sm text-text-100 font-medium">4 列</div>
                <div className="text-sm text-text-50">M: 16px / G: 12px</div>
              </div>
              <div className="grid grid-cols-4 gap-4 p-4 md:p-6 border-b border-border-subtle/50 hover:bg-bg-elevated-2/30 transition-colors">
                <div className="font-bold text-text-100">Tablet <span className="text-text-50 font-normal text-sm ml-2 hidden md:inline">(md)</span></div>
                <div className="font-mono text-sm text-primary">≥ 768px</div>
                <div className="text-sm text-text-100 font-medium">8 列</div>
                <div className="text-sm text-text-50">M: 24px / G: 16px</div>
              </div>
              <div className="grid grid-cols-4 gap-4 p-4 md:p-6 hover:bg-bg-elevated-2/30 transition-colors">
                <div className="font-bold text-text-100">Desktop <span className="text-text-50 font-normal text-sm ml-2 hidden md:inline">(lg)</span></div>
                <div className="font-mono text-sm text-primary">≥ 1024px</div>
                <div className="text-sm text-text-100 font-medium">12 列</div>
                <div className="text-sm text-text-50">M: Auto / G: 24px</div>
              </div>
            </div>

            {/* Game Card Aspect Ratio Demo */}
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl">
              <h3 className="text-xl font-bold text-text-100 mb-2">游戏卡片等比例缩放 (Aspect Ratio)</h3>
              <p className="text-sm text-text-50 mb-8 leading-relaxed max-w-3xl">
                针对游戏封面的特殊尺寸需求，我们使用 <code className="text-primary font-mono bg-primary/10 px-1.5 py-0.5 rounded border border-primary/20">aspect-[100/134]</code> 锁定比例。
              ...
              <div className="space-y-10">
                {/* 固定尺寸对比 */}
                <div>
                  <h4 className="text-sm font-bold text-text-50 uppercase tracking-widest mb-4 border-b border-border-subtle pb-2">固定尺寸基准 (Fixed Size Reference)</h4>
                  <div className="flex flex-wrap gap-8 items-end">
                    {/* Mobile Size */}
                    <div className="flex flex-col gap-3">
                      <div className="w-[100px] aspect-[100/134] bg-bg-elevated-2 rounded-xl  relative overflow-hidden group shadow-lg">
                    ...
                    {/* Desktop Size */}
                    <div className="flex flex-col gap-3">
                      <div className="w-[118px] aspect-[100/134] bg-bg-elevated-2 rounded-xl  relative overflow-hidden group shadow-lg">
                        <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-transparent"></div>
        ...
        <section>
          ...
          <div className="space-y-8">
            {/* 8.1 尺寸基准 (Size Scale) */}
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl">
              <h3 className="text-xl font-bold text-text-100 mb-2">8.1 绝对尺寸档位 (Size Tiers)</h3>
              <p className="text-sm text-text-50 mb-6 leading-relaxed">
            ...
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl overflow-x-auto">
              <h3 className="text-xl font-bold text-text-100 mb-2">8.2 变体与状态矩阵 (Variants & States Matrix)</h3>
              <p className="text-sm text-text-50 mb-6 leading-relaxed">
                所有按钮必须具备完整的交互状态反馈。禁止出现“悬浮没反应”或“禁用态看不清”的情况。
                <br/><span className="text-warning">注意：同组按钮（如弹窗底部的确认和取消）必须保持相同的圆角规则，禁止混用 Pill 和 Rounded。</span>
              </p>

              <div className="min-w-[850px]">
                <div className="grid grid-cols-6 gap-4 mb-4 text-xs font-bold text-text-50 uppercase tracking-wider border-b border-border-subtle pb-2">
                  ...
                  <div className="text-center text-primary">交互预览 (Preview)</div>
                </div>
                ...
                <div className="grid grid-cols-6 gap-4 items-center py-4 border-b border-border-subtle/50">
                  <div>
                    <div className="font-bold text-text-100">Secondary</div>
                    <div className="text-xs text-text-50">次级操作，深灰填充</div>
                  </div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-bg-elevated-2  text-text-100 font-bold rounded-lg text-sm">Secondary</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-bg-elevated-3  text-text-100 font-bold rounded-lg text-sm">Hover</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-bg-elevated-2 brightness-90  text-text-100 font-bold rounded-lg text-sm scale-[0.97] transition-all duration-75">Active</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-bg-elevated-2  text-text-50 font-bold rounded-lg text-sm opacity-50 cursor-not-allowed">Disabled</button></div>
                  <div className="flex justify-center border-l border-border-subtle/50 pl-4"><button className="h-10 px-4 bg-bg-elevated-2 hover:bg-bg-elevated-3 active:bg-bg-elevated-2 active:brightness-90  text-text-100 font-bold rounded-lg text-sm active:scale-[0.97] transition-all duration-75">Try Me</button></div>
                </div>

                {/* Ghost */}
                <div className="grid grid-cols-6 gap-4 items-center py-4 border-b border-border-subtle/50">
                  <div>
                    <div className="font-bold text-text-100">Ghost / Outline</div>
                    <div className="text-xs text-text-50">幽灵按钮，透明底色</div>
                  </div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-transparent border border-border-strong text-text-100 font-bold rounded-lg text-sm">Ghost</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-white/5 border border-border-strong text-text-100 font-bold rounded-lg text-sm">Hover</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-white/10 border border-border-strong text-text-100 font-bold rounded-lg text-sm scale-[0.97] transition-all duration-75">Active</button></div>
                  <div className="flex justify-center"><button className="h-10 px-4 bg-transparent border border-border-strong text-text-50 font-bold rounded-lg text-sm opacity-50 cursor-not-allowed">Disabled</button></div>
                  <div className="flex justify-center border-l border-border-subtle/50 pl-4"><button className="h-10 px-4 bg-transparent hover:bg-white/5 active:bg-white/10 border border-border-strong text-text-100 font-bold rounded-lg text-sm active:scale-[0.97] transition-all duration-75">Try Me</button></div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl">
              <h3 className="text-xl font-bold text-text-100 mb-2">8.3 按钮圆角的环境绑定 (Contextual Radius)</h3>
              <p className="text-sm text-text-50 mb-8 leading-relaxed max-w-3xl">
                按钮的圆角<strong className="text-error">绝对不允许</strong>脱离环境单独设定。必须严格遵循“同心圆法则”：<br/>
                <code className="text-primary font-mono bg-primary/10 px-1.5 py-0.5 rounded border border-primary/20">按钮圆角 = 容器圆角 - 容器内边距 (Padding)</code>
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* 场景 A: 嵌套在卡片中 */}
                <div className="space-y-3">
                  <h4 className="text-sm font-bold text-text-50 uppercase tracking-widest">场景 A: 嵌套在卡片中</h4>
                  <div className="bg-bg-elevated-2 p-4 rounded-2xl ">
        ...
        {/* 9. Interaction & Shadows (NEW) */}
        <section>
          <SectionTitle description="阴影在扁平化布局中起到定义'海拔高度'的作用。深色模式下，我们结合背景提亮、微光边框和纯黑大半径阴影来模拟悬浮感。品牌色外发光 (Glow) 仅用于核心转化路径或选中状态。">
            9. 交互与阴影 (Interaction & Shadows)
          </SectionTitle>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Standard Elevation */}
            <div className="bg-bg-elevated-1 p-8 rounded-2xl  shadow-[0_8px_30px_rgba(0,0,0,0.6)] flex flex-col items-center justify-center text-center gap-4">
              <div className="w-12 h-12 rounded-full bg-bg-elevated-3 flex items-center justify-center mb-2">
                <Layers className="w-5 h-5 text-text-50" />
              </div>
              <h4 className="text-base font-bold text-text-100">Standard Elevation</h4>
              <p className="text-xs text-text-50 leading-relaxed">
                Used for all game cards and content containers.<br/>
                <span className="text-primary/80 mt-1 inline-block">纯黑大半径阴影 + 微弱边框</span>
              </p>
            </div>

            {/* Active / Glow State */}
            <div className="bg-bg-elevated-1 p-8 rounded-2xl border border-primary shadow-[0_0_24px_rgba(255,108,68,0.15)] flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden">
              <div className="absolute inset-0 bg-primary/5"></div>
              <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center mb-2 relative z-10 shadow-[0_0_16px_rgba(255,108,68,0.4)]">
                <CheckCircle2 className="w-5 h-5 text-primary" />
              </div>
              <h4 className="text-base font-bold text-primary relative z-10">Active / Glow State</h4>
              <p className="text-xs text-text-50 leading-relaxed relative z-10">
                Used for active selections and primary CTAs.<br/>
                <span className="text-primary/80 mt-1 inline-block">品牌色弥散发光 + 内部微光背景</span>
              </p>
            </div>

            {/* Hover Feedback */}
            <div className="bg-bg-elevated-1 p-8 rounded-2xl  shadow-[0_8px_30px_rgba(0,0,0,0.6)] flex flex-col items-center justify-center text-center gap-4 transition-all duration-300 hover:-translate-y-2 hover:border-text-50 hover:shadow-[0_16px_40px_rgba(0,0,0,0.9)] cursor-pointer group">
              <div className="w-12 h-12 rounded-full bg-bg-elevated-3 flex items-center justify-center mb-2 group-hover:bg-bg-elevated-2 transition-colors">
                <MousePointer2 className="w-5 h-5 text-text-50 group-hover:text-text-100 transition-colors" />
              </div>
              <h4 className="text-base font-bold text-text-100 group-hover:text-white transition-colors">Hover Feedback</h4>
              <p className="text-xs text-text-50 leading-relaxed">
                Subtle border color change and scale transform.<br/>
                <span className="text-primary/80 mt-1 inline-block">(请将鼠标悬浮在此卡片上体验)</span>
              </p>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Top Nav */}
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Menu className="w-5 h-5 text-primary" /> 顶部导航栏 (Top App Bar)
                </h3>
                ...
                <div className="relative h-40 bg-bg-base rounded-xl  overflow-hidden">
                  <div className="absolute inset-x-0 top-0 h-14 bg-bg-nav/80 backdrop-blur-md border-b border-border-subtle flex items-center justify-between px-4 z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-text-100">首页</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <Search className="w-5 h-5 text-text-50" />
                      <div className="relative">
                        <Bell className="w-5 h-5 text-text-50" />
                  ...
                  <div className="p-4 pt-20 space-y-3 opacity-30">
                    <div className="h-20 bg-bg-elevated-2 rounded-lg"></div>
                    <div className="h-20 bg-bg-elevated-2 rounded-lg"></div>
                  </div>
                </div>

                {/* Rationale */}
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">毛玻璃效果 (Backdrop Blur)</strong>：深色模式下，纯色导航栏会显得极其沉闷。使用半透明背景加高斯模糊，能在长列表滚动时透出底层内容的色彩轮廓，保持空间层次感（Z-axis），这是 Apple HIG 强烈推荐的做法。<br/>
                    <strong className="text-text-100">高度规范</strong>：移动端通常固定在 44px-56px，确保触控热区达标。
                  </p>
                </div>
              </div>
              ...
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Home className="w-5 h-5 text-primary" /> 底部标签栏 (Bottom Tab Bar)
                </h3>
                ...
                <div className="relative h-40 bg-bg-base rounded-xl  overflow-hidden flex flex-col justify-end">
                  <div className="h-16 bg-bg-tab border-t border-border-subtle flex items-center justify-around px-2 pb-safe">
                    <div className="flex flex-col items-center gap-1 cursor-pointer">
                      ...
                      <span className="text-[10px] font-bold text-primary">大厅</span>
                    </div>
                    <div className="flex flex-col items-center gap-1 cursor-pointer">
                      <Gamepad2 className="w-6 h-6 text-text-50" />
                      <span className="text-[10px] font-medium text-text-50">游戏</span>
                    </div>
                    <div className="flex flex-col items-center gap-1 cursor-pointer">
                      <User className="w-6 h-6 text-text-50" />
                      <span className="text-[10px] font-medium text-text-50">我的</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">拇指热区 (Thumb Zone)</strong>：根据人体工学，屏幕底部是单手握持时最容易触达的区域。核心导航必须放在这里。<br/>
                    <strong className="text-text-100">状态对比</strong>：选中态必须有极强的视觉对比（品牌色 + 粗体），未选中态需压暗（50% 透明度），降低视觉噪音。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Search className="w-5 h-5 text-primary" /> 搜索框 (Search Input)
                </h3>
                ...
                <div className="p-6 bg-bg-base rounded-xl  flex items-center justify-center">
                  <div className="w-full max-w-sm h-10 bg-bg-elevated-2 rounded-full  flex items-center px-4 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition-all">
                    ...
                    <XCircle className="w-4 h-4 text-text-25 hover:text-text-50 cursor-pointer transition-colors" />
                  </div>
                </div>
                ...
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">视觉启示 (Affordance)</strong>：搜索框几乎清一色使用<strong className="text-primary">胶囊圆角 (Pill)</strong>。这能在潜意识里将其与普通的表单输入框（通常是 8px/12px 圆角）区分开来，暗示这是一个全局探索入口。<br/>
                    <strong className="text-text-100">即时反馈</strong>：右侧必须提供一键清除 (Clear) 按钮，提升移动端输入效率。
                  </p>
                </div>
              </div>
              ...
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <ChevronDown className="w-5 h-5 text-primary" /> 下拉筛选 (Dropdown)
                </h3>
                ...
                <div className="p-6 bg-bg-base rounded-xl  flex flex-col items-center justify-center h-48 relative">
                  ...
                  {/* Popover Mock */}
                  <div className="absolute top-20 w-48 bg-bg-elevated-2 rounded-xl  shadow-[0_8px_30px_rgba(0,0,0,0.8)] overflow-hidden z-20">
                    <div className="p-1">
                      <div className="px-3 py-2 text-sm text-primary font-bold bg-primary/10 rounded-lg cursor-pointer flex justify-between items-center">
                        全部游戏类型 <CheckCircle2 className="w-4 h-4" />
                      </div>
                      <div className="px-3 py-2 text-sm text-text-100 hover:bg-bg-elevated-3 rounded-lg cursor-pointer transition-colors">体育赛事</div>
                      <div className="px-3 py-2 text-sm text-text-100 hover:bg-bg-elevated-3 rounded-lg cursor-pointer transition-colors">真人娱乐</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">上下文保留 (Context Retention)</strong>：使用浮层而不是页面跳转，能大幅减少用户的认知负荷。<br/>
                    <strong className="text-text-100">Z轴海拔</strong>：下拉菜单必须带有极强的阴影 (Standard Elevation)，确保它在视觉上完全凌驾于页面其他元素之上。选中项需用品牌色高亮。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <AlertTriangle className="w-5 h-5 text-primary" /> 中心弹窗 (Center Modal)
                </h3>
                ...
                <div className="relative h-64 bg-bg-base rounded-xl  overflow-hidden flex items-center justify-center">
                  ...
                  <div className="relative z-20 w-64 bg-bg-elevated-2 rounded-2xl  shadow-[0_16px_40px_rgba(0,0,0,0.8)] p-5 flex flex-col gap-4">
                    <div className="text-center space-y-2">
                      <h4 className="text-base font-bold text-text-100">确认退出登录？</h4>
                ...
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">强阻断 (Blocking)</strong>：中心弹窗用于极其重要的二次确认。必须配备 <code className="text-primary">bg-black/60</code> 的全屏遮罩，强制聚焦用户视线。<br/>
                    <strong className="text-text-100">同心圆法则</strong>：注意弹窗容器是 24px (2xl) 圆角，内部按钮严格使用了 12px (xl) 圆角，保持视觉平行。
                  </p>
                </div>
              </div>
              ...
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Layers className="w-5 h-5 text-primary" /> 底部抽屉 (Bottom Sheet)
                </h3>
                ...
                <div className="relative h-64 bg-bg-base rounded-xl  overflow-hidden flex flex-col justify-end">
                  ...
                  <div className="relative z-20 w-full bg-bg-elevated-2 rounded-t-3xl border-t border-border-subtle shadow-[0_-8px_30px_rgba(0,0,0,0.6)] p-6 pt-3 flex flex-col gap-4">
                    ...
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="text-lg font-bold text-text-100">快捷充值</h4>
                      <X className="w-5 h-5 text-text-50 cursor-pointer" />
                    </div>
                        ...
                        <button
                          ...
                          className={`h-12 rounded-xl border font-bold flex items-center justify-center text-sm transition-all active:scale-[0.98] ${
                            activeAmount10_3 === amt
                              ...
                              : 'bg-bg-elevated-1 border-border-subtle text-text-100 hover:bg-bg-elevated-2'
                          }`}
                        >
                          {amt}
                        </button>
                ...
                <div className="bg-bg-elevated-2 p-4 rounded-xl border-l-4 border-primary">
                  <h4 className="text-sm font-bold text-text-100 mb-1">设计依据 (Rationale)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">单手操作友好 (One-Handed Use)</strong>：现代 App 替代复杂中心弹窗的首选方案。从底部滑出，完美契合大屏手机的单手操作。<br/>
                    <strong className="text-text-100">拖拽指示器 (Drag Handle)</strong>：顶部必须有一个 1.5px 高的胶囊条，暗示用户可以向下拖拽关闭，符合物理直觉。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8">
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Ruler className="w-5 h-5 text-primary" /> 弹窗与浮层尺寸规范 (Overlay Sizing Guidelines)
                </h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="relative h-64 bg-bg-base rounded-xl  overflow-hidden flex items-center justify-center">
                      ...
                      <div className="relative z-20 w-[80%] max-w-[280px] bg-bg-elevated-2 rounded-2xl  shadow-[0_16px_40px_rgba(0,0,0,0.8)] flex flex-col max-h-[80%]">
                        <div className="p-4 border-b border-border-subtle shrink-0">
                          <h5 className="font-bold text-text-100 text-sm text-center">固定宽度，动态高度</h5>
                        </div>
                        <div className="p-4 overflow-y-auto text-xs text-text-50 space-y-2">
                          <p>宽度：移动端通常为 <code className="text-primary">w-[calc(100vw-32px)]</code> 或最大 <code className="text-primary">320px-400px</code>。避免过宽导致视线移动过长。</p>
                          <p>高度：随内容自适应，但必须设置最大高度（如 <code className="text-primary">max-h-[80vh]</code>）。</p>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="relative h-64 bg-bg-base rounded-xl  overflow-hidden flex flex-col items-center justify-start pt-8 gap-4">
                      ...
                      <div className="w-[90%] max-w-[300px] min-h-[48px] bg-bg-elevated-3 rounded-xl  shadow-lg flex items-center px-4">
                        ...
                        <span className="text-xs text-text-100 font-medium">操作成功（单行文本）</span>
                      </div>
                      {/* Multi line toast */}
                      <div className="w-[90%] max-w-[300px] min-h-[48px] bg-bg-elevated-3 rounded-xl  shadow-lg flex items-start p-4">
                        <AlertTriangle className="w-4 h-4 text-warning shrink-0 mr-2 mt-0.5" />
                        <span className="text-xs text-text-100 font-medium leading-relaxed">网络连接不稳定，请检查您的网络设置后重试。（多行文本，高度自适应）</span>
                      </div>
                    </div>
                    <div className="text-xs text-text-50 space-y-2 mt-2">
                      <p>宽度：通常限制最大宽度（如 <code className="text-primary">max-w-sm</code>），避免横跨整个屏幕。</p>
                      <p>高度：<strong className="text-text-100">必须设置最小高度</strong>（如 <code className="text-primary">min-h-[48px]</code> 或 <code className="text-primary">56px</code>）。</p>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="relative h-64 bg-bg-base rounded-xl  overflow-hidden flex flex-col justify-end">
                      ...
                      <div className="relative z-20 w-full bg-bg-elevated-2 rounded-t-3xl border-t border-border-subtle shadow-[0_-8px_30px_rgba(0,0,0,0.6)] flex flex-col h-[85%]">
                        ...
                        <div className="px-6 pb-2 border-b border-border-subtle shrink-0">
                          <h5 className="font-bold text-text-100 text-sm">游戏筛选</h5>
                        </div>
                        <div className="p-6 overflow-y-auto text-xs text-text-50 space-y-4">
                          <div className="h-8 bg-bg-elevated-3 rounded-lg"></div>
                          <div className="h-8 bg-bg-elevated-3 rounded-lg"></div>
                          <div className="h-8 bg-bg-elevated-3 rounded-lg"></div>
                          <div className="h-8 bg-bg-elevated-3 rounded-lg"></div>
                        </div>
                    ...
                    <div className="text-xs text-text-50 space-y-2 mt-2">
                      <p>宽度：移动端 <code className="text-primary">100%</code>，平板/桌面端通常限制最大宽度并居中。</p>
                      <p>高度：随内容自适应，但<strong className="text-text-100">绝对不能达到 100% 屏幕高度</strong>。</p>
                      <p>最高点：通常最高限制在 <code className="text-primary">90vh</code> 或距离顶部留出安全区+24px的间距。必须露出背后的半透明遮罩，给用户“我还在当前页面，向下滑动即可关闭”的明确心理暗示。</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 gap-8">
              <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl flex flex-col gap-6">
                <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                  <Layers className="w-5 h-5 text-primary" /> 分段控制器 / 标签栏 (Segmented Control)
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs text-text-50">
                      <strong className="text-error">问题所在：</strong>滑块贴死边缘导致空间逼仄；左侧滑块被迫变成“左圆右直”的畸形怪状，破坏了组件的独立性。
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <p className="text-xs text-text-50">
                      <strong className="text-success">设计依据：</strong>引入 <code className="text-primary">4px (p-1)</code> 的内边距，形成“轨道与滑块”的物理隐喻。严格遵守同心圆法则（外容器 12px - 内边距 4px = 滑块 8px），滑块保持完整的圆角矩形，视觉呼吸感极佳。
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl shadow-xl flex flex-col gap-8">
              <h3 className="text-lg font-bold text-text-100 flex items-center gap-2">
                <Layout className="w-5 h-5 text-primary" /> 页面与容器规范 (Page & Surface Layout)
              </h3>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative aspect-video bg-bg-base rounded-2xl border border-border-subtle overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 flex-1 flex flex-col gap-4">
                      <div className="h-8 w-1/3 bg-bg-elevated-1 rounded-lg animate-pulse"></div>
                      <div className="flex-1 grid grid-cols-2 gap-4">
                        <div className="bg-bg-elevated-1 rounded-xl"></div>
                        <div className="bg-bg-elevated-1 rounded-xl"></div>
                      </div>
                    </div>

                    <div className="p-4 bg-bg-elevated-1/50 border-t border-border-subtle">
                      <div className="grid grid-cols-2 gap-4 text-[10px] font-mono">
                        <div className="flex flex-col gap-1">
                          <span className="text-text-25">Background</span>
                          <span className="text-text-100">color/bg/canvas (#141414)</span>
                        </div>
                        <div className="flex flex-col gap-1">
                          <span className="text-text-25">Main Margin</span>
                          <span className="text-text-100">spacing/16 (16px)</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">用途：</strong>页面基础背景。所有内容均承载于此画布之上。默认文字颜色为 <code className="text-primary">color/text/primary</code>。
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-bold text-text-50 uppercase tracking-widest">2. Surface Container</h4>
                    <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded font-bold">ELEVATION LAYERS</span>
                  </div>
                  
                  <div className="grid grid-cols-1 gap-4">
                    {/* Level 1 */}
                    <div className="bg-bg-elevated-1 p-4 md:p-6 rounded-2xl border border-border-subtle flex items-center justify-between group hover:border-primary/50 transition-colors">
                    ...
                    {/* Level 2 */}
                    <div className="bg-bg-elevated-2 p-4 rounded-2xl flex items-center justify-between group hover:ring-1 hover:ring-primary/30 transition-all">
                      <div className="space-y-1">
                        <div className="text-sm font-bold text-text-100">Surface / Level 2</div>
                        <div className="text-[10px] text-text-50 font-mono">bg/surface/2 | radius/16 | p/16</div>
                      </div>
                      <div className="w-10 h-10 bg-bg-elevated-1 rounded-xl flex items-center justify-center text-xs font-bold text-text-25">L2</div>
                    </div>

                    {/* Level 3 */}
                    <div className="bg-bg-elevated-3 p-3 rounded-xl flex items-center justify-between group hover:brightness-110 transition-all">
                      <div className="space-y-1">
                        <div className="text-sm font-bold text-text-100">Surface / Level 3</div>
                        <div className="text-[10px] text-text-50 font-mono">bg/surface/3 | radius/12 | p/12</div>
                      </div>
                      <div className="w-8 h-8 bg-bg-elevated-2 rounded-lg flex items-center justify-center text-[10px] font-bold text-text-25">L3</div>
                    </div>
                  </div>

                  <p className="text-xs text-text-50 leading-relaxed">
                    <strong className="text-text-100">用途：</strong>卡片、浮层、区块容器。通过背景色的明度提升来模拟物理世界中的“海拔”高度。Level 1 通常带有弱边边框。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl flex flex-col">
              <div className="flex justify-between items-center mb-6 border-b border-border-subtle pb-4">
                <h3 className="text-lg font-bold text-text-100 uppercase tracking-wide">Bet Slip</h3>
                <span className="bg-primary/20 text-primary px-3 py-1 rounded-full text-xs font-bold">LIVE</span>
              </div>
              
              <div className="space-y-4 flex-1">
                {/* Note: Outer is rounded-xl (12px), inner elements are rounded-lg (8px) or smaller */}
                <div className="bg-bg-elevated-2 p-4 rounded-xl hover:border-primary/50 transition-colors cursor-pointer">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-sm font-bold text-text-100">Manchester Utd vs Arsenal</span>
                    <span className="text-error text-xs font-bold flex items-center gap-1"><XCircle size={12}/> 89'</span>
                  </div>
                  <div className="flex justify-between items-center mt-4">
                    <span className="text-text-50 text-sm font-medium">Match Winner</span>
                    <div className="bg-bg-elevated-3 px-4 py-2 rounded-lg text-primary font-extrabold text-lg">
                      2.45
                    </div>
                  </div>
                </div>
            ...
            {/* Alerts & Feedback */}
            <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl space-y-4 flex flex-col">
              <h3 className="text-lg font-bold text-text-100 uppercase tracking-wide border-b border-border-subtle pb-4 mb-6">System Feedback</h3>
              ...
              <div className="space-y-4 flex-1">
                ...
                <div className="flex items-start gap-3 p-4 rounded-xl bg-success-light border border-success/20">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-50 mt-1 font-medium">Your ticket ID is #89237492.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 rounded-xl bg-warning-light border border-warning/20">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-50 mt-1 font-medium">The odds for your selection have decreased to 2.10.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 rounded-xl bg-error-light border border-error/20">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-50 mt-1 font-medium">Please deposit funds to place this bet.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-bg-elevated-1 p-6 rounded-2xl  shadow-xl lg:col-span-2">
              <div className="flex justify-between items-center mb-6 border-b border-border-subtle pb-4">
                <h3 className="text-lg font-bold text-text-100 uppercase tracking-wide">Complex Modal (Overlay Sizing & Segmented Control)</h3>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-3 relative h-[500px] bg-bg-base rounded-xl  overflow-hidden flex items-center justify-center">
                  ...
                  {/* Modal Container */}
                  <div className="relative z-20 w-[90%] max-w-[400px] bg-bg-elevated-2 rounded-2xl  shadow-[0_16px_40px_rgba(0,0,0,0.8)] flex flex-col max-h-[90%]">
                    {/* Header */}
                    <div className="p-5 border-b border-border-subtle shrink-0 flex justify-between items-center">
                      <h4 className="font-bold text-text-100 text-lg">Deposit Funds</h4>
                      <button className="text-text-50 hover:text-text-100 transition-colors"><XCircle size={20} /></button>
                    </div>
                        ...
                        {['$100', '$250', '$500'].map(amt => (
                          <button
                            ...
                            className={`h-10 rounded-lg font-bold text-sm transition-all active:scale-[0.98] border ${
                              activeAmount11 === amt
                                ? 'bg-primary/10 border-primary text-primary'
                                : 'bg-bg-elevated-3 hover:bg-bg-elevated-1 border-border-subtle text-text-100'
                            }`}
                          >
                            {amt}
                          </button>
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-bg-elevated-2 p-4 rounded-xl ">
                    <h4 className="text-sm font-bold text-primary mb-2 flex items-center gap-2"><Info size={16}/> 中心弹窗 vs 底部抽屉</h4>
                    <p className="text-xs text-text-50 leading-relaxed">
                      <strong className="text-text-100">中心弹窗 (Center Modal):</strong> 用于强阻断、需要用户集中注意力完成的独立任务（如：充值、确认删除、复杂表单）。<br/><br/>
                      <strong className="text-text-100">底部抽屉 (Bottom Sheet):</strong> 用于轻量级交互、上下文相关的选择或操作（如：分享菜单、筛选器、快速回复），在移动端更易于单手操作。
                    </p>
                  </div>

                  <div className="bg-bg-elevated-2 p-4 rounded-xl ">
                    <h4 className="text-sm font-bold text-primary mb-2 flex items-center gap-2"><XCircle size={16}/> 什么时候需要关闭按钮？</h4>
                    <p className="text-xs text-text-50 leading-relaxed">
                      1. <strong className="text-text-100">必须有:</strong> 当弹窗内容较长（可能滚动隐藏底部取消按钮），或者是一个纯信息展示/表单填写弹窗时，右上角必须提供关闭按钮。<br/>
                      2. <strong className="text-text-100">可省略:</strong> 当弹窗是一个简单的对话框（如：确认/取消），且底部操作区始终可见时，右上角的关闭按钮可以省略，依赖底部的“取消”或点击遮罩层关闭。
                    </p>
                  </div>
          ...
          <div className="bg-bg-elevated-1 p-6 md:p-8 rounded-2xl  shadow-xl space-y-8">
            <div className="flex items-start gap-4 p-6 bg-error-light/10 border border-error/30 rounded-xl">
              ...
              <div>
                ...
                <p className="text-sm text-text-100 leading-relaxed mb-4">
                  在协同办公中，最可怕的不是缺乏设计，而是<strong className="text-error">无视现有规范，不断“发明”新设计</strong>。
                ...
                <p className="text-sm text-text-100 leading-relaxed">
                  <strong className="text-warning">请尊重维护人员的劳动成果！</strong> 维护一套严谨的组件库和设计规范是非常辛苦的。每一个 4px 的间距、每一个圆角的嵌套、每一种颜色的透明度，都是经过深思熟虑的。随意打破这些规则，意味着维护者需要花费数倍的时间去填补这些“技术债务”。
                </p>
              </div>
            </div>

            <div>
              <h3 className="text-base font-bold text-text-100 mb-4 flex items-center gap-2">
                <Layers className="w-5 h-5 text-primary" /> 正确的构建思维：从底层容器到零碎组件
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-5 bg-bg-elevated-2 rounded-xl ">
                  <div className="w-8 h-8 rounded-full bg-primary/20 text-primary flex items-center justify-center font-bold mb-3">1</div>
                  <h4 className="font-bold text-text-100 text-sm mb-2">确定底层容器 (Containers)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                ...
                <div className="p-5 bg-bg-elevated-2 rounded-xl ">
                  ...
                  <h4 className="font-bold text-text-100 text-sm mb-2">应用基础组件 (Foundations)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
                ...
                <div className="p-5 bg-bg-elevated-2 rounded-xl ">
                  ...
                  <h4 className="font-bold text-text-100 text-sm mb-2">填充零碎元素 (Fragments)</h4>
                  <p className="text-xs text-text-50 leading-relaxed">
            ...
            <div className="bg-bg-elevated-2 p-6 rounded-xl border-l-4 border-success">
              <h3 className="text-base font-bold text-text-100 mb-2 flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-success" /> 专业使用指南
              </h3>
              <ul className="text-sm text-text-50 space-y-3 list-disc list-inside">
                <li><strong className="text-text-100">先查阅，后设计：</strong> 在开始任何新页面的设计或开发前，先在本文档中寻找可复用的模式。</li>
                <li><strong className="text-text-100">克制创新：</strong> 如果你觉得现有的组件无法满足需求，请先与规范维护者沟通，探讨是否可以通过组合现有组件来实现，而不是立刻创建一个全新的样式。</li>
                <li><strong className="text-text-100">拥抱约束：</strong> 规范的存在就是为了限制自由。在 8px 和 10px 之间犹豫时，请毫不犹豫地选择规范中定义的 8px。</li>
                <li><strong className="text-text-100">全局视野：</strong> 你正在设计的不是一个孤立的页面，而是整个庞大系统的一部分。每做一次特殊处理，都在增加系统的熵值。</li>
              </ul>
            </div>
          </div>
        </section>
